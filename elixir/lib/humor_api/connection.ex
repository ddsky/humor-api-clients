...
defmodule HumorAPI.Connection do
  @moduledoc """
  Handle Tesla connections for HumorAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, HumorAPI.Connection,
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:humorapi, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - HumorAPI 1.0 - elixir"
        )
      )
