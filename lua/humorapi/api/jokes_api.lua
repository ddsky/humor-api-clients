...
local humorapi_search_jokes_200_response = require "humorapi.model.search_jokes_200_response"
...
function jokes_api:analyze_joke(body)
	...
	if http_status:sub(1,1) == "2" then
		...
		return humorapi_analyze_joke_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function jokes_api:random_joke(keywords, include_tags, exclude_tags, min_rating, max_length)
	...
	if http_status:sub(1,1) == "2" then
		...
		return humorapi_random_joke_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function jokes_api:search_jokes(keywords, include_tags, exclude_tags, Number_, min_rating, max_length, offset)
	...
	if http_status:sub(1,1) == "2" then
		...
		return humorapi_search_jokes_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
