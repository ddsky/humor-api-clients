...
-- | display the config
instance P.Show HumorConfig where
  show c =
    T.printf
...
-- | constructs a default HumorConfig
--
-- configHost:
--
...
-- @"humor/0.1.0.0"@
--
newConfig :: IO HumorConfig
newConfig = do
    logCxt <- initLogContext
    return $ HumorConfig
        { configHost = "https://api.humorapi.com"
        , configUserAgent = "humor/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => HumorConfig -> auth -> HumorConfig
addAuthMethod config@HumorConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: HumorConfig -> IO HumorConfig
...
withStderrLogging :: HumorConfig -> IO HumorConfig
...
withNoLogging :: HumorConfig -> HumorConfig
...
data HumorRequest req contentType res accept = HumorRequest
  { rMethod  :: NH.Method   -- ^ Method of HumorRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of HumorRequest
  , rParams   :: Params -- ^ params of HumorRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (HumorRequest req contentType res accept) NH.Method
rMethodL f HumorRequest{..} = (\rMethod -> HumorRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (HumorRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f HumorRequest{..} = (\rUrlPath -> HumorRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (HumorRequest req contentType res accept) Params
rParamsL f HumorRequest{..} = (\rParams -> HumorRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (HumorRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f HumorRequest{..} = (\rAuthTypes -> HumorRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => HumorRequest req contentType res accept -> param -> HumorRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: HumorRequest req contentType res accept -> param -> HumorRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: HumorRequest req contentType res accept -> param -> HumorRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> HumorRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = HumorRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: HumorRequest req contentType res accept -> [NH.Header] -> HumorRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: HumorRequest req contentType res accept -> [NH.HeaderName] -> HumorRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => HumorRequest req contentType res accept -> HumorRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => HumorRequest req contentType res accept -> HumorRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: HumorRequest req contentType res accept -> [NH.QueryItem] -> HumorRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: HumorRequest req contentType res accept -> WH.Form -> HumorRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: HumorRequest req contentType res accept -> NH.Part -> HumorRequest req contentType res accept
...
_setBodyBS :: HumorRequest req contentType res accept -> B.ByteString -> HumorRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: HumorRequest req contentType res accept -> BL.ByteString -> HumorRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => HumorRequest req contentType res accept -> P.Proxy authMethod -> HumorRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: HumorConfig
    -> a
    -> HumorRequest req contentType res accept
    -> IO (HumorRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> HumorConfig
  -> IO (HumorRequest req contentType res accept)
_applyAuthMethods req config@(HumorConfig {configAuthMethods = as}) =
  foldlM go req as
  where
